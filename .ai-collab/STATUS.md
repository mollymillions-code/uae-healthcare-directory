...
- **[Claude Code] 2026-04-05T23:15:00+04:00** — Fixed broken "Book a Demo" form on zavis.ai/contact and zavis.ai/book-a-demo. Root cause: (1) form was POSTing to dead Vercel URL (zavis-onboarding.vercel.app returning 402), (2) CSP `connect-src` directive blocked requests to the EC2 backend. Fix: updated fallback API URL in ContactPageClient.tsx to `clientops.zavisinternaltools.in`, added `clientops.zavisinternaltools.in` to CSP connect-src in next.config.mjs, configured matching `LEADS_WEBHOOK_SECRET` on both EC2 shared .env.local and onboarding .env.local. Two commits deployed via blue-green pipeline (blue→green→blue). COMPLETE.
