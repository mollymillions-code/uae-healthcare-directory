...
export default async function InsurerDetailPage({ params }: Props) {
  ...
  const faqs = [
    ...
    {
      question: `Is ${profile.name} the UAE healthcare regulator or HAAD compliant?`,
      answer: `${profile.name} is regulated by ${profile.regulators.map((r) => r.toUpperCase()).join(" and ")}. ${profile.regulators.includes("dha") ? "The UAE healthcare regulator (Dubai) compliance means all plans meet the Essential Benefits Package required for Dubai residents and workers." : ""}${profile.regulators.includes("haad") ? `${profile.regulators.includes("dha") ? " Additionally, " : ""}Health Authority Abu Dhabi (HAAD/DoH) oversight ensures plans meet Abu Dhabi's mandatory Thiqa and basic scheme requirements.` : ""}${profile.regulators.includes("mohap") ? " the UAE healthcare regulator registration applies for the Northern Emirates." : ""}`,
    },
    {
