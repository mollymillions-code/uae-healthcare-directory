...
export function generateMetadata({ params }: Props): Metadata {
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        "en-AE": `${base}/professionals/${params.category}/${spec.slug}/specialists`,
        "ar-AE": `${base}/ar/professionals/${params.category}/${spec.slug}/specialists`,
      },
    },
    openGraph: {
      title: `أخصائيو ${spec.nameAr} في دبي — ${count.toLocaleString("ar-AE")} مرخّص من DHA`,
      description: `${count.toLocaleString("ar-AE")} أخصائي في ${spec.nameAr} في دبي. دليل شامل مصدره السجل الطبي شريان.`,
      url: `${base}/ar/professionals/${params.category}/${spec.slug}/specialists`,
      type: "website",
    ...
    <div dir="rtl" lang="ar" className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <JsonLd
        data={{
          ...
          "@type": "MedicalWebPage",
          name: `أخصائيو ${spec.nameAr} في دبي`,
          description: `${specialists.length.toLocaleString("ar-AE")} أخصائي في ${spec.nameAr} مرخّصون من هيئة صحة دبي.`,
          url: `${base}/ar/professionals/${params.category}/${spec.slug}/specialists`,
          mainContentOfPage: {
            "@type": "WebPageElement",
      ...
      <div className="mb-10">
        ...
        <h1 className="font-['Bricolage_Grotesque',sans-serif] font-medium text-[28px] sm:text-[34px] text-[#1c1c1c] tracking-tight mb-2">
          أخصائيو {spec.nameAr} في دبي
        </h1>
        <p className="font-['Geist_Mono',monospace] text-xs text-[#006828] font-medium tracking-wider uppercase mb-4">
          {specialists.length.toLocaleString("ar-AE")} أخصائي مرخّص من هيئة صحة دبي
        </p>

        {/* Specialist grade explanation */}
        <div className="border-r-4 border-[#006828] bg-[#006828]/[0.04] py-5 px-6 mb-6">
          <p className="font-['Geist',sans-serif] text-sm text-black/60 leading-relaxed">
            <strong className="text-[#1c1c1c]">ما هو الأخصائي؟</strong> في نظام ترخيص هيئة صحة دبي،
            &ldquo;الأخصائي&rdquo; هو طبيب أو طبيب أسنان أتمّ تدريباً متقدماً في تخصصه
            بعد الحصول على درجته الطبية الأساسية، ويحمل مؤهلاً دراسياً عالياً معترفاً به
            (مثل: شهادة البورد، الزمالة، أو ما يعادلها). الأخصائيون مؤهّلون لتشخيص الحالات
            وعلاجها باستقلالية ضمن نطاق تخصصهم. يوجد حالياً{" "}
            {specialists.length.toLocaleString("ar-AE")} أخصائي في {spec.nameAr}{" "}
            يمارسون في {stats.totalFacilities.toLocaleString("ar-AE")} منشأة في دبي.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          {[
            { value: specialists.length.toLocaleString("ar-AE"), label: ar.professionals.specialists },
      ...
      <div className="mb-10 border border-black/[0.06] p-5">
        <div className="flex items-center gap-3 mb-3 border-b-2 border-[#1c1c1c] pb-3">
          <h2 className="font-['Bricolage_Grotesque',sans-serif] font-medium text-[18px] sm:text-[20px] text-[#1c1c1c] tracking-tight">
            الأخصائيون والاستشاريون في {spec.nameAr}
          </h2>
        </div>
        <div className="grid grid-cols-2 gap-6 mb-4">
          ...
          <div className="text-center">
            <p className="text-3xl font-bold text-[#1c1c1c]">
              {consultants.length.toLocaleString("ar-AE")}
            </p>
            <p className="font-['Geist',sans-serif] text-xs text-black/40 mt-1">
        ...
        <p className="font-['Geist',sans-serif] text-sm text-black/60 leading-relaxed mb-3">
          يوجد في دبي {specialists.length.toLocaleString("ar-AE")} أخصائي و{consultants.length.toLocaleString("ar-AE")} استشاري
          في {spec.nameAr}. يحتل الاستشاريون الدرجة الأعلى، وعادةً ما يتطلب ذلك أكثر من 8 سنوات
          من الخبرة بعد التخصص، مع قدرة على الإشراف على الأخصائيين وقيادة الأقسام.
        </p>
        {consultants.length > 0 && (
          <Link
            href={`/ar/professionals/${cat.slug}/${spec.slug}/consultants`}
            className="font-['Bricolage_Grotesque',sans-serif] text-sm font-semibold text-[#006828] hover:underline"
          >
            عرض جميع الاستشاريين البالغ عددهم {consultants.length.toLocaleString("ar-AE")} في {spec.nameAr} &larr;
          </Link>
        )}
      </div>
        ...
        <>
          <div className="flex items-center gap-3 mb-6 border-b-2 border-[#1c1c1c] pb-3">
            <h2 className="font-['Bricolage_Grotesque',sans-serif] font-medium text-[20px] sm:text-[24px] text-[#1c1c1c] tracking-tight">
              أبرز المنشآت لأخصائيي {spec.nameAr}
            </h2>
          </div>
          <div className="mb-12">
      ...
      <p className="font-['Geist',sans-serif] text-xs text-black/40 mb-4">
        ...
        {specialists.length.toLocaleString("ar-AE")} أخصائي في {spec.nameAr}، {ar.professionals.sortedAlphabetically}.
      </p>
        ...
        <div className="flex flex-col gap-2">
          ...
          {consultants.length > 0 && (
            <Link
              ...
              className="font-['Geist',sans-serif] text-sm text-[#006828] hover:underline"
            >
              استشاريو {spec.nameAr} ({consultants.length.toLocaleString("ar-AE")}) &larr;
            </Link>
          )}
          <Link
