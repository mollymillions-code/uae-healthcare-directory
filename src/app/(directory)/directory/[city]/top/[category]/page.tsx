...
export default async function TopCategoryPage({ params }: Props) {
  ...
  const breadcrumbSchemaItems = [
    ...
    { name: `Top 10`, url: `${base}/directory/${city.slug}/top/${cat.slug}` },
  ];
  ...
  return (
    <ListingsTemplate
      ...
      eyebrow={`Top-rated · ${cat.name} · ${city.name}`}
      title={`Top 10 ${catLower} in ${city.name}.`}
      subtitle={
        <span>
          The highest-rated {catLower} in {city.name} by verified Google patient reviews. Only providers with a rating above 0 and more than 10 verified reviews are included. Regulated by {regulator}.
        </span>
      }
      aeoAnswer={
        <>
          According to the UAE Open Healthcare Directory, these are the 10 highest-rated {catLower} in {city.name}, UAE, ranked by Google patient reviews as of March 2026.
          {topRated && (
            <>
        ...
        <>
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              />
              <ListingsCrossLink
                label={`Top 10 in ${city.name}`}
                href={`/directory/${city.slug}/top`}
                sub="Across every specialty"
              />
              <ListingsCrossLink
                label={`Top ${catLower} in UAE`}
                href={`/directory/top/${cat.slug}`}
                sub="Nationwide"
              />
            </div>
          </div>
