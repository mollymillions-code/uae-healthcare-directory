...
export default async function TopAreaCategoryPage({ params }: Props) {
  ...
  const pageUrl = `${base}/directory/${city.slug}/${area.slug}/top/${cat.slug}`;
      ...
      <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <h1 className="font-['Bricolage_Grotesque',sans-serif] font-medium text-[28px] sm:text-[34px] text-[#1c1c1c] tracking-tight mb-3">
            Top 10 {cat.name} in {area.name}, {city.name}, UAE
          </h1>
          <p className="font-['Geist',sans-serif] text-black/40 leading-relaxed mb-4">
            The {cat.name.toLowerCase()} below are the highest-rated in {area.name}, {city.name} by verified Google patient reviews,
          ...
          <div className="border-l-4 border-[#006828] bg-[#006828]/[0.04] rounded-xl py-5 px-6 mb-6" data-answer-block="true">
            <p className="font-['Geist',sans-serif] text-black/40 leading-relaxed">
              According to the UAE Open Healthcare Directory, these are the {top10.length} highest-rated {catLower} in {area.name}, {city.name}, UAE,
        ...
        <section className="mb-10">
          <div className="flex items-center gap-3 mb-6 border-b-2 border-[#1c1c1c] pb-3">
            <h2 className="font-['Bricolage_Grotesque',sans-serif] font-medium text-[20px] sm:text-[24px] text-[#1c1c1c] tracking-tight">Ranked List &mdash; {cat.name} in {area.name}, {city.name}</h2>
          </div>
        ...
        <section className="mb-10">
          <p className="font-['Geist',sans-serif] text-sm text-black/40">
            ...
            >
              Browse all {cat.name.toLowerCase()} in {area.name} &rarr;
            </Link>
          </p>
          <p className="font-['Geist',sans-serif] text-sm text-black/40 mt-2">
            ...
            <Link
              ...
              className="text-[#006828] hover:underline font-medium"
            >
              Top 10 {cat.name} in {city.name} &rarr;
            </Link>
          </p>
        </section>

        <FaqSection faqs={faqs} title={`Top ${cat.name} in ${area.name}, ${city.name} — FAQ`} />
      </div>
    </>
  );
}
