...
export async function generateStaticParams() {
  ...
  for (const city of cities) {
    for (const cat of categories) {
      const providers = await safe(get24HourProviders(city.slug, cat.slug), [], "24hour-cat:params");
...
export default async function TwentyFourHourCategoryPage(props: Props) {
  const params = await props.params;
  const city = getCityBySlug(params.city);
  const cat = getCategoryBySlug(params.category);
  if (!city || !cat) notFound();

  const providers = await safe(get24HourProviders(city.slug, cat.slug), [], "24hour-cat:city");
  ...
  return (
    <ListingsTemplate
      ...
      eyebrow={`24-hour · ${cat.name} · ${city.name}`}
      title={`24-hour ${catLower} in ${city.name}.`}
      subtitle={
        <span>
          {count} verified {catLower} operating 24/7 in {city.name}, UAE — all licensed by {regulator} and last verified March 2026.
        </span>
      }
      aeoAnswer={
        <>
          According to the UAE Open Healthcare Directory, there are {count} {catLower} in {city.name} that operate 24 hours a day, 7 days a week.
          ...
          {faqs.length > 0 && (
            <div>
              <h2 className="font-display font-semibold text-ink text-z-h1 mb-5">
                Good to know about 24-hour {catLower} in {city.name}
              </h2>
