      ...
      <div className="container-tc py-8">
        ...
        {subcategories.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-6">
            {subcategories.map((sub) => (<Link key={sub.slug} href={`/uae/${city.slug}/${category.slug}/${sub.slug}`} className="badge-outline px-3 py-1.5 text-sm hover:bg-accent-muted">{sub.name}</Link>))}
          </div>
        )}
        ...
        {areas.length > 0 && (
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
              {areas.map((a) => (<Link key={a.slug} href={`/uae/${city.slug}/${a.slug}/${category.slug}`} className="inline-block bg-light-100 text-dark text-sm px-3 py-1.5 border border-light-200 hover:border-accent hover:bg-accent-muted transition-colors">{a.name}</Link>))}
            </div>
          </div>
        )}
    ...
    return (
      <div className="container-tc py-8">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
            {categories.map((cat) => (<Link key={cat.slug} href={`/uae/${city.slug}/${area.slug}/${cat.slug}`} className="inline-block bg-light-100 text-dark text-sm px-3 py-1.5 border border-light-200 hover:border-accent hover:bg-accent-muted transition-colors">{cat.name}</Link>))}
          </div>
        </div>
      ...
      <div className="container-tc py-8">
        ...
        {providers.length > 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <Link href={`/uae/${city.slug}/${category.slug}`} className="text-accent text-sm">View all {category.name.toLowerCase()} in {city.name} &rarr;</Link>
          </div>
        )}
                ...
                <div className="border border-light-200 p-6">
                  ...
                  <div className="space-y-3">
                    {nearbyProviders.map((np) => (
                      <Link key={np.id} href={`/uae/${np.citySlug}/${np.categorySlug}/${np.slug}`} className="block text-sm hover:text-accent transition-colors">
