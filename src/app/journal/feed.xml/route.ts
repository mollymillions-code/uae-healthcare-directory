...
export async function GET() {
  ...
  const items = articles
    .map((article) => {
      ...
      <link>${base}/journal/${article.slug}</link>
      <guid isPermaLink="true">${base}/journal/${article.slug}</guid>
...
<rss version="2.0"
  ...
  <channel>
    <title>UAE Healthcare Journal</title>
    <link>${base}/journal</link>
    <description>Real-time coverage of UAE healthcare: regulatory updates, new openings, financial analysis, health tech, events, and workforce trends.</description>
