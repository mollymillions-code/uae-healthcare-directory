...
import { professionalsIndex, adminChanges } from "@/lib/db/schema";
...
const EDITABLE_FIELDS = new Set([
  "name", "nameAr", "displayTitle", "education", "educationDescription",
  "primaryFacilityName", "primaryFacilitySlug", "primaryCitySlug",
  "specialty", "specialtySlug", "discipline", "level",
  "licenseType", "licenseCount", "photoUrl", "photoConsent",
  "status",
]);
...
export async function PATCH(
  ...
  try {
    const current = await db.select().from(professionalsIndex).where(eq(professionalsIndex.id, Number(params.id))).limit(1);
    ...
    await db.update(professionalsIndex).set(updates).where(eq(professionalsIndex.id, Number(params.id)));
    ...
    console.error("[admin] PATCH /professionals/[id] failed:", err);
