...
export const INSURANCE_EDITORIAL: InsuranceEditorialEntry[] = [
  // ─── Thiqa (AD-only, UAE nationals) ────────────────────────────────────────
  {
    ...
    isMedical: true,
    editorialCopyEn:
      "Thiqa is the Government of Abu Dhabi's premium health programme for Emirati nationals, administered by Daman on behalf of the UAE healthcare regulator (Abu Dhabi). Coverage is restricted to UAE nationals holding an Abu Dhabi family book and is not available to expatriates or to residents of other emirates. Thiqa provides near-zero co-pay at all UAE-licensed (Abu Dhabi) facilities in Abu Dhabi and Al Ain, and at a gated list of private facilities inside the emirate. Outside Abu Dhabi, reciprocal network access is limited to the UAE healthcare regulator facilities that have signed Thiqa direct-billing agreements — typically major hospital groups such as Mediclinic, NMC, Aster and Burjeel. Dental, optical and maternity are included without sub-limit for the standard programme; international referrals require the UAE healthcare regulator pre-authorisation. Because Thiqa is geographically bounded, Zavis only surfaces Thiqa pages for Abu Dhabi and Al Ain, and suppresses Thiqa facet URLs elsewhere to avoid thin content.",
    editorialCopyAr:
  ...
  // Daman Basic (the UAE healthcare regulator-mandated floor cover) is referenced inline.
  {
    slug: "daman",
    nameEn: "Daman",
    nameAr: "ضمان",
