...
export default function ResearchPageClient({ reports }: { reports: ReportMeta[] }) {
  ...
  const restReports = filtered.length > 1 ? filtered.slice(1) : []
  ...
  return (
    <div className="min-h-screen bg-[#f8f8f6]">
              ...
              {categories.slice(0, 5).map(cat => (
                <button
                  ...
                  className={`text-[13px] font-medium transition-colors ${
                    activeCategory === cat
                      ? 'text-[#006828]'
                      : 'text-black/40 hover:text-[#006828]'
                  }`}
                >
                  {cat}
                </button>
        ...
        <div className="flex items-center gap-4 border-b border-black/[0.06] pb-4">
          ...
          <div className="flex gap-2 overflow-x-auto scrollbar-hide">
            {categories.map(cat => (
              <button
                key={cat}
                onClick={() => handleCategoryClick(cat)}
                className={`inline-block text-[12px] font-medium px-3 py-1 rounded-full whitespace-nowrap transition-colors cursor-pointer ${
                  activeCategory === cat
                    ? 'bg-[#006828]/10 text-[#006828]'
                    : 'text-black/50 bg-black/[0.03] hover:bg-[#006828]/[0.08] hover:text-[#006828]'
                }`}
              >
                {cat}
              </button>
          ...
          {activeCategory && (
            <button
              ...
              className="text-[11px] font-medium text-black/30 hover:text-black/60 transition-colors whitespace-nowrap"
            >
              Clear filter
            </button>
          )}
      ...
      <section className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 pb-16 lg:pb-24">
        ...
        {restReports.length === 0 && filtered.length <= 1 && (
          <p className="text-center text-[14px] text-black/30 mt-8">No other reports in this category yet.</p>
        )}
      </section>
