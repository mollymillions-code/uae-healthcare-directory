/**
 * Translate provider descriptions and reviewSummaries to Arabic for indices 2500-2999.
 * Uses Gemini 2.0 Flash in batches of 20 providers.
 * Keeps facility names, area names, city names, numbers, ratings, DHA/DOH/MOHAP in English.
 */
...
const chunk = allProviders.slice(2500, 3000);
console.log(`Loaded ${chunk.length} providers (indices 2500-2999)`);

// Output path
const outputPath = join(projectRoot, "scripts/arabic-chunks/ar-2500-2999.json");
...
async function processBatch(items, attempt = 1) {
  try {
    ...
    if (!parsed) {
      if (attempt < 3) {
        console.log(
          `  Retry attempt ${attempt + 1} for batch starting at index ${items[0].index}`
        );
...
for (let i = 0; i < chunk.length; i += BATCH_SIZE) {
  ...
  process.stdout.write(
    `Translating ${batchStart}-${batchEnd} (${Object.keys(results).length + batchItems.length}/500)... `
  );
...
if (missing.length > 0) {
  console.log(
    `Missing indices (${missing.length}): ${missing.slice(0, 20).join(", ")}${missing.length > 20 ? "..." : ""}`
  );
} else {
  console.log("All 500 indices present.");
}
